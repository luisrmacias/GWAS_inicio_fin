...
status <- status[status$V2 %in% mestizos$IID,]
all.equal(status$V2, mestizos$IID)
##true
	tst <- wilcox.test(mestizos$NAT ~ status[,2]==1)
	pval = tidy(tst)$p.value
	plt1 <- ggplot(mestizos, aes(x=status[,2]==1, y=NAT))+ 
	geom_boxplot()
	plt1 + geom_text(aes(x=0.7, y = 0.82, label=paste('P-value:', format.pval(pval, digits=2))))
boxplot(mestizos$NAT ~ status[,2], col=c("honeydew3", "indianred"), notch=TRUE, names=c("Casos", "Controles"), ylab="Proporción de componente ancestral indígena", cex.lab=1.15, cex.axis=1.15)
text(0.7, 0.51, bquote(p== .(signif(pval,4))), cex=1.15)
